...
class WebScraperService
{
    ...
    public function scrapeForTenantParallel(int $tenantId, ?int $scraperConfigId = null): array
    {
        ...
        // Log finale sessione
        ScraperLogger::sessionCompleted($this->sessionId, [
    ...
    private function extractContent(string $html, string $url): ?array
    {
        ...
        $extractedContent['source_page_title'] = $this->prepareSourcePageTitle(
            $html,
            $this->currentConfig?->titleStrategy() ?? TitleStrategy::default()
        );
    ...
    private function cleanupContent(string $content): string
    {
        ...
        foreach ($lines as $line) {
            ...
            if (preg_match('/^(home|contact|about|privacy|terms|menu|search|login|register|©|\d{4})$/i', $trimmedLine)) {
                continue;
            }
    ...
    {
        $dom = new \DOMDocument;
        $cleanHtml = $this->sanitizeHtmlForDom($html);

        $previousUseErrors = libxml_use_internal_errors(true);

        try {
            $dom->loadHTML($cleanHtml, LIBXML_NOERROR | LIBXML_NOWARNING);
        } finally {
            libxml_clear_errors();
    ...
    /**
     * Rimuove caratteri di controllo problematici prima del parsing DOM
     */
    private function sanitizeHtmlForDom(string $html): string
    {
        // Rimuove caratteri di controllo ASCII (eccetto \n, \r, \t)
        $html = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/u', '', $html) ?? $html;

        // Rimuove i caratteri di controllo C1 (0x80-0x9F) spesso presenti in Windows-1252
        $html = preg_replace('/[\x{0080}-\x{009F}]/u', '', $html) ?? $html;
    ...
    private function prepareSourcePageTitle(string $html, TitleStrategy $strategy): ?string
    {
        $rawTitle = null;
        ...
        try {
            ...
            if (@$dom->loadHTML($html)) {
                ...
                $rawTitle = match ($strategy) {
                    TitleStrategy::TITLE => $this->firstNodeText($xpath, '//title'),
                    TitleStrategy::H1 => $this->firstNodeText($xpath, '//h1'),
                    TitleStrategy::H1_H2 => $this->concatNodeText($xpath, '//h1 | //h2'),
                };
            }

            libxml_clear_errors();
        } catch (\Throwable $exception) {
            \Log::debug('scraper.title_strategy.crawler_error', [
    ...
    private function concatNodeText(\DOMXPath $xpath, string $expression): ?string
    {
        $nodes = $xpath->query($expression);
        if (! $nodes || $nodes->length === 0) {
            return null;
                ...
                if ($content) {
                    ...
                    foreach ($links as $link) {
                        if (! isset($this->visitedUrls[$link]) &&
                            ...
                            \Log::debug('📥 [PARALLEL-SCRAPE] Queuing child URL', [
                                'parent_url' => $url,
                                'child_url' => $link,
                                'next_depth' => $depth + 1,
                                'max_depth' => $config->max_depth,
