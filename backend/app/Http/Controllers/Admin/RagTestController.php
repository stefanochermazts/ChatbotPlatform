    ...
    public function run(Request $request, KbSearchService $kb, OpenAIChatService $chat, MilvusClient $milvus)
    {
        ...
        if ((bool) ($data['with_answer'] ?? false)) {
            ...
            $contextResult = $contextBuilder->build($citations, $tenantId, [
                'compression_enabled' => false, // Disable LLM compression for RAG Tester
            ]);
...
- Usa il titolo originale del documento, non descrizioni generiche'];
    ...
    private function calculateContentQualityScore(array $citation): float
    {
        ...
        if (preg_match('/\b(?:telefono|email|indirizzo|orari?|contatti?)\b/i', $chunkText)) {
            $score += 0.15;
        }
    ...
    private function calculateIntentMatchScore(array $citation): float
    {
        ...
        // Telefoni
        if (preg_match('/(?:\+39\s*)?0\d{1,3}[\s\.\-]*\d{6,8}/', $chunkText)) {
            $score += 0.1;
        }

        // Email
        if (preg_match('/@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}/', $chunkText)) {
            $score += 0.1;
        }

        // Indirizzi
        if (preg_match('/\b(?:via|viale|piazza|corso|largo)\s+[A-Z]/i', $chunkText)) {
            $score += 0.1;
        }

        // Orari strutturati
        if (preg_match('/\d{1,2}[:\.]?\d{0,2}\s*[-–—]\s*\d{1,2}[:\.]?\d{0,2}/', $chunkText)) {
            $score += 0.1;
        }
    ...
    private function calculateSourceAuthorityScore(array $citation): float
    {
        ...
        switch (strtolower($documentType)) {
            case 'pdf': $score += 0.2;
                break;  // PDF spesso più formali
            case 'doc':
            case 'docx': $score += 0.15;
                break;
            case 'txt':
            case 'md': $score += 0.1;
                break;
